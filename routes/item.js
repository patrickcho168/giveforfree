...
module.exports = function(app) {
  ...
  app.get('/api/friendItems/:lastItemId/:loadNum', ensureLogin.ensureLoggedIn(),
    function(req, res) {
      ...
      console.log(req.user.fbFriendsId);
      if (lastSeenItem === 0) { // NEW QUERY
  ...
  app.get('/item/:id', ensureLogin.ensureLoggedIn(),
    function(req, res){
      ...
      db.Item.where({itemID: itemId}).fetch().then(function(itemData) {
        if (itemData === null) { // No such item
          res.redirect('/');
        }
        // Is it my item?
        var giverId = parseInt(itemData.attributes.giverID);
        var mine = parseInt(req.user.appUserId) === giverId;
        // TODO: THINK OF LOGIC
        // Should item be shown even if not friend's or mine?
        var friend;
